...
#' @export
#' @import data.table
krasddpcams__plot_RAF_invitro_cor<-function(
  input=input,
  assay_name=assay_name,
  colour_scheme
  ){
  input_ddG<-krasddpcams__read_ddG(ddG = input,assay_sele = assay_name)
  ...
  ggplot2::ggplot(christina_mochi,ggplot2::aes(x=ddG_ITC,y=`mean_kcal/mol`,label=mt))+
    ...
    ggplot2::annotate("text",x=0.5,y=2.0,,label = paste0("r = ",round(sqrt(summary(christina_mochi_m)$r.squared),2)) ,size=7*0.35)+
