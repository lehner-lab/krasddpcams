...
#' @return Nothing
#' @export
Plot_drivenmt_wt_fitness_color_fig3<-function(input = input,
                              driven_mt = driven_mt,
                              ddG_RAFRBD=ddG_RAFRBD,
                              anno=anno,
                              assay_sele=assay_sele){
  # annotation
  ddG1<-Read_ddG(ddG = ddG_RAFRBD,
                 assay_sele = assay_sele)
  weighted_mean_ddG1<-Get_weighted_mean_abs_ddG(ddG=ddG_RAFRBD,assay_sele = assay_sele)
  ...
  # plot
  ggplot(mix_anno[AA_Pos1<65,],aes(x=nor_fitness,y=i.nor_fitness,color=mutation_type))+
    geom_point(size=0.1)+
    annotate("text",x=-0.5,y=0.2,label = paste0("r = ",round(sqrt(summary(r_model)$r.squared),2)) ,size=7*0.35,color="black")+
    geom_smooth(method=lm, se=T,size=0.1,color="black")+
    annotate("text",x=-0.5,y=0.1,label = paste0("r = ",round(sqrt(summary(r_model_allosteric)$r.squared),2)) ,size=7*0.35,color=colour_scheme[["orange"]])+
    geom_smooth(method=lm, se=T,size=0.1,color=colour_scheme[["orange"]])+
    xlab("Binding fitness in the background of WT")+
    ylab(paste("Binding fitness in the background of",driven_mt))+
    geom_vline(xintercept = input_WT[WT==TRUE,nor_fitness])+
    geom_hline(yintercept = input_WT[mt1==driven_mt,nor_fitness])+
    theme_classic2()+
