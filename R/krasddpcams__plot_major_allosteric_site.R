...
#' @export
#' @import data.table
krasddpcams__plot_major_allosteric_site<-function(
  ddG=ddG,
  anno=anno,
  ...
  weighted_mean_ddG<-krasddpcams__get_weighted_mean_abs_ddG_mutcount(ddG=ddG,
  ...
  allosteric_list[["p"]]<-ggplot2::ggplot()+
    ...
    ggrepel::geom_text_repel(data=data_plot[site_type=="Major allosteric site",],ggplot2::aes(x=get(paste0("scHAmin_ligand_",assay_name)),
                                                                                                              y=mean,label=Pos),nudge_y=0.05,color=colour_scheme[["green"]],size=7*0.35)+
    ggrepel::geom_text_repel(data=data_plot[site_type=="Allosteric GTP pocket site",],ggplot2::aes(x=get(paste0("scHAmin_ligand_",assay_name)),
                                                                                                                    y=mean,label=Pos),nudge_y=0.05,color=colour_scheme[["blue"]],size=7*0.35)+
    ggplot2::ylab(paste("Weighted mean |ddG|(kcal/mol)"))+
