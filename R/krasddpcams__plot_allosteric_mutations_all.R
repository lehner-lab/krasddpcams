...
#' @export
#' @import data.table 
krasddpcams__plot_allosteric_mutations_all<-function(
  ddG=ddG,
  anno=anno,
  ...
  data_plot_mutation<-within(data_plot_mutation,
                             mutation_type<-factor(mutation_type,
                                                   levels=c("Orthosteric site huge differences",
                                                            ...
                                                            "Other mutation")))
  ggplot2::ggplot()+
    ggplot2::geom_rect(data=rect_input,ggplot2::aes(ymin=y_min,ymax=y_max,xmin=xstart-0.5,xmax=xend+0.5),fill="black",alpha=0.08)+
    ggplot2::geom_rect(data=rect_alpha,ggplot2::aes(ymin=y_min,ymax=y_max,xmin=xstart-0.5,xmax=xend+0.5),fill="black",alpha=0.05)+
