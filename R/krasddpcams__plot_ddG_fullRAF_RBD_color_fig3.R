...
#' @export
#' @import data.table 
krasddpcams__plot_ddG_fullRAF_RBD_color_fig3<-function(
  ddG1=ddG1,
  ddG2=ddG2,
  ...
  ddG_RAFRBD1<-krasddpcams__read_ddG(ddG = ddG_RAFRBD,
                 assay_sele = assay_sele)
  # ddG_RAFRBD
  weighted_mean_ddG1<-krasddpcams__get_weighted_mean_abs_ddG(ddG=ddG_RAFRBD,assay_sele = "RAF")
  ...
  #plot
  ggplot2::ggplot()+
    ggplot2::geom_point(data=all_ddG_dc_anno,ggplot2::aes(`mean_kcal/mol_a`,`mean_kcal/mol_b`,color=mutation_type),size=0.1)+
    ggplot2::annotate("text",x=0,y=2,,label = paste0("r = ",round(sqrt(summary(ddG_lm)$r.squared),2)) ,size=7*0.35)+
    ggplot2::annotate("text",x=0,y=1.5,,label = paste0("r = ",round(sqrt(summary(ddG_lm_allosteric)$r.squared),2)) ,size=7*0.35,color=colour_scheme[["orange"]])+
    ggplot2::geom_smooth(data=all_ddG_dc_anno,ggplot2::aes(`mean_kcal/mol_a`,`mean_kcal/mol_b`),method=lm, se=T,size=0.1,color="black")+
    ggplot2::geom_smooth(data=all_ddG_dc_anno[mutation_type=="Allosteric mutation",],ggplot2::aes(`mean_kcal/mol_a`,`mean_kcal/mol_b`),method=lm, se=T,size=0.1,color=colour_scheme[["orange"]])+
