...
Write_tables5<-function(
  ...
  Get_fdr<-function(ddG=ddG){
    ddG[!is.na(Pos_real),ddG_fdr:=p.adjust(doubledeepms__pvalue(`mean_kcal/mol`,`std_kcal/mol`),method="BH")]
  ...
  ddG8<-Read_ddG(ddG8,assay8)
  ddG8<-Get_fdr(ddG=ddG8)
  
  all_ddG<-rbind(ddG1,ddG2,ddG3,ddG4,ddG5,ddG6,ddG7,ddG8)
  all_ddG[,id:=mt]
  all_ddG[mt=="NANANA",id:="WT"]
  write.xlsx(all_ddG[,.(Pos_real,wt_codon,mt_codon,`mean_kcal/mol`,`std_kcal/mol`,`ci95_kcal/mol`,id,assay,ddG_conf)],
