...
#' @return Nothing
#' @export
Plot_RAL_invitro_cor_ITC<-function(ddG=ddG,
  ...
  ggplot(christina_mochi_RAL,aes(x=ddG_ITC,y=`mean_kcal/mol`,label=mt))+
    geom_smooth(method=lm, se=T,size=0.1,color=colour_scheme[["red"]])+
    geom_point(size=0.1)+
    geom_hline(yintercept=0)+
    geom_vline(xintercept=0)+
    geom_abline(intercept = 0,slope=1,linetype="dashed")+
    xlab("Binding ddG to RALGDS (in vitro/ITC)\n(kcal/mol)")+
