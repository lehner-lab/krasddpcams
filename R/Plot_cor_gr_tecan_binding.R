...
#' @return Nothing
#' @export
Plot_cor_gr_tecan_binding<-function(tecandata=tecandata, plasmidid = plasmidid,single=single){
  # read OD data
  ...
  ggplot(data_gr[assay.x=="binding"&assay.y=="RAF"&mutation!="wt2",],aes(x=mean,y=nor_fitness,label=mutation))+
    geom_smooth(method=lm, se=T,size=0.1,color=colour_scheme[["red"]])+
    xlab("Growth rate of individual measurements")+
    ylab("fitness of BindingPCA")+
    geom_pointrange(aes(ymin=nor_fitness-nor_fitness_sigma, ymax=nor_fitness+nor_fitness_sigma),size=0.3,color="gray")+
    geom_errorbarh(aes(xmin=mean-sd, xmax=mean+sd,height = 0),size=0.3,color="gray")+
    geom_point(size=2)+
    geom_text_repel(size=7*0.35)+
    annotate("text",x=0.08,y=0.23,label = paste0("r = ",round(sqrt(summary(bind_lm_gr)$r.squared),2)) ,size=7*0.35)+
    theme_classic2()+
