...
#' @export
#' 
Plot_allosteric_mut_distance_2colors<-function(
  ...
  ggplot2::ggplot(data_plot_mutation,aes(x=scHAmin_ligand_RAF,y=`mean_kcal/mol`))+
    ...
    geom_point(aes(color=colors_type2),size=1*0.35)+
    geom_vline(xintercept = 5,linetype =2)+
    scale_color_manual(values = c(colour_scheme[["red"]],colour_scheme[["blue"]],colour_scheme[["purple"]],"black"))+
    ylab("Binding free energy change \n(kcal/mol)")+
    xlab(expression(paste("Distance to binding parnter ("*ring(A)*")")))+
    theme_classic()+
