...
#' RALGDS in vitro(ITC) cor Function
#' 
...
#' @export
#' @import data.table
krasddpcams__plot_RAL_invitro_cor_ITC<-function(
  ddG=ddG,
  ...
  ggplot2::ggplot(christina_mochi_RAL,ggplot2::aes(x=ddG_ITC,y=`mean_kcal/mol`,label=mt))+
    ggplot2::geom_smooth(method=lm, se=T,size=0.1,color=colour_scheme[["red"]])+
    ggplot2::geom_point(size=0.1)+
    ggplot2::geom_hline(yintercept=0)+
    ggplot2::geom_vline(xintercept=0)+
    ggplot2::geom_abline(intercept = 0,slope=1,linetype="dashed")+
    ggplot2::xlab("Binding ddG to RALGDS (in vitro/ITC)\n(kcal/mol)")+
    ...
    ggrepel::geom_text_repel(hjust=0.5, vjust=2,size=7*0.35)+
    ggplot2::annotate("text",x=0.75,y=1.75,
             label = paste0("r = ",round(sqrt(summary(christina_mochi_RAL_m_ITC)$r.squared),3)),
             size=7*0.35 )+
