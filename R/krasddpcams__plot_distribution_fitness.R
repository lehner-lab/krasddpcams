...
#' @export
#' @import data.table
krasddpcams__plot_distribution_fitness<-function(
  input=input,
  ...
  input_single_pos <- krasddpcams__pos_id(input_single,wt_aa = wt_aa)
  input_single_pos[,position:=AA_Pos1]
  input_single_pos[,WT_AA:=wtcodon1]
  #input_single_pos[,fitness_fdr:=p.adjust(krasddpcams__pvalue(nor_fitness,nor_fitness_sigma),method="BH")]
