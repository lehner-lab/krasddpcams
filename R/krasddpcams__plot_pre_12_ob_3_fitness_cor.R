...
#' A Function to plot predicted fitness from replicates 1&2 against observed fitness replicate 3
#' 
...
#' @export
#' @import data.table
krasddpcams__plot_pre_12_ob_3_fitness_cor<-function(
  ...
  ggplot2::ggplot(plot_dt,ggplot2::aes(observed_fitness, predicted_fitness)) +
    ...
    ggplot2::scale_fill_gradientn(colours = c("white", "black"), trans = "log10",guide = ggplot2::guide_colorbar(barwidth = 0.5,barheight = 1.5)) +
    # ggplot2::geom_point(alpha = 1/10) +
    ggplot2::xlab("Observed fitness (Rep. 3)") +
    ggplot2::ylab("Predicted fitness (Reps. 1&2)") +
    ggplot2::annotate("text",x=-1,y=r_y,
